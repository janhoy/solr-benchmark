...
- Q: Should the 8 new/ported telemetry devices use "Solr-" prefixed class names and `--telemetry solr-*` flags? → A: No. Port and edit the existing OSB device classes using the same names as OSB (e.g., `SegmentStats` not `SolrSegmentStats`, `--telemetry segment-stats` not `--telemetry solr-segment-stats`, `FlightRecorder` not `SolrFlightRecorder`, `jfr` not `solr-jfr`). The old OpenSearch-specific behavior MUST be replaced with Solr-equivalent behavior. No "Solr-" prefix on device class names or `--telemetry` flag values for the ported devices.
...
8. **Given** the user specifies `--telemetry segment-stats`, **When** the benchmark completes, **Then** per-collection segment stats (numDocs, deletedDocs, segmentCount, indexHeapUsageBytes) are captured via the Luke API and appear in the benchmark report.
9. **Given** a SolrCloud benchmark with `--telemetry shard-stats`, **When** the benchmark runs, **Then** per-shard document counts, index sizes, and replica states are polled every 60s and included in the time-series metrics.
10. **Given** the `from-distribution` or `docker` pipeline with `--telemetry jfr` or `--telemetry gc`, **When** Solr starts, **Then** the appropriate JVM flags are injected into `SOLR_OPTS` and the corresponding log files (profile.jfr, gc.log) are written to the benchmark log directory.
...
6. **Given** an OpenSearch Benchmark workload directory, **When** the user runs `solr-benchmark run --workload-path /path/to/os-workload`, **Then** the tool detects the OSB format and exits with a clear ERROR message: "This workload is in OpenSearch Benchmark format. Run `solr-benchmark convert-workload --workload-path <src> --output-path <dest>` to convert it to Solr format first."
7. **Given** a previously converted Solr workload (`CONVERTED.md` present), **When** the user runs `solr-benchmark run --workload-path /path/to/workload-solr`, **Then** the benchmark runs normally — no detection or conversion logic is invoked.
...
- **FR-018a**: When loading a workload for benchmarking, the tool MUST detect whether the workload is in OpenSearch Benchmark format (`"indices"` key present, uses `create-index`/`delete-index` operation types, etc.) or in Solr-native format (`"collections"` key present, uses Solr-native operations). Detection logic lives in `osbenchmark/solr/conversion/detector.py`.
- **FR-018b**: If an OpenSearch Benchmark workload is detected at run time, the tool MUST abort immediately with a clear ERROR message instructing the user to run `solr-benchmark convert-workload` first. NO automatic conversion is performed during a benchmark run. The benchmark runs only against Solr-native workloads.
...
- **FR-018f**: The search runner MUST NOT perform any OpenSearch DSL translation at query execution time. All translation happens at workload conversion time (pre-run). The runner accepts only: (a) no body — classic Solr params (`q`, `fq`, `sort`, `rows`), or (b) a body with a `"query"` key whose value is a string — Solr JSON Query DSL POSTed to `/query`. If `body["query"]` is a dict (OpenSearch DSL), the runner MUST raise an error — the workload has reached the runner unconverted, which should never happen given FR-018b aborts at load time.
...
- **FR-019**: The tool MUST collect Solr-specific performance metrics during benchmark runs via the following telemetry devices. All devices query Solr admin APIs and write named metrics into the time-series results store. Target Solr version: 9.x and 10.x.
  ...
  **Phase 2 — operational monitoring devices (SHOULD implement):**
  - **SegmentStats** (user-selectable, `segment-stats`): Edit the existing OSB `SegmentStats` device to call `GET /solr/{collection}/admin/luke?numTerms=0` (Solr Luke API) instead of OpenSearch's `/_cat/segments`. One-shot collection at benchmark end per collection. Captures: numDocs, maxDoc, deletedDocs, segmentCount, indexHeapUsageBytes. Works with all pipelines.
  - **ShardStats** (user-selectable, `shard-stats`): New device (no OSB equivalent) using OSB naming convention. Per-shard document counts, index sizes, and replica state, polled every 60 seconds. API: `GET /solr/admin/collections?action=CLUSTERSTATUS` for topology + `GET /solr/admin/cores?action=STATUS&core={core}` for leader index metrics. SolrCloud only; MUST skip gracefully on standalone.
  - **ClusterEnvironmentInfo** (internal, always-on): Edit the existing OSB `ClusterEnvironmentInfo` device to call `GET /api/node/system` (Solr system info) + CLUSTERSTATUS instead of OpenSearch endpoints. Captures: Solr version, JVM version/vendor, OS name/version, CPU count, collection count, total shard and replica count. Stored as benchmark metadata (not time-series). Works with all pipelines.

  **Phase 3 — JVM instrumentation and operational tools (MAY implement):**
  These devices require a provisioned pipeline (`from-distribution`, `docker`, `from-sources`). On `benchmark-only` pipeline they MUST log a WARNING and skip without error.
  - **FlightRecorder** (user-selectable, `jfr`): Edit the existing OSB `FlightRecorder` device so its `instrument_java_opts()` flags are injected into `SOLR_OPTS` instead of OpenSearch JVM options. JFR flags are JVM-agnostic and remain unchanged: `-XX:+UnlockDiagnosticVMOptions -XX:+DebugNonSafepoints -XX:StartFlightRecording=...,dumponexit=true,filename={log_dir}/profile.jfr`. Provisioner-only.
  - **Gc** (user-selectable, `gc`): Edit the existing OSB `Gc` device so its `instrument_java_opts()` flags are injected into `SOLR_OPTS`. GC log flags are JVM-agnostic: `-Xlog:{gc_config}:file={log_dir}/gc.log:utctime,uptimemillis,level,tags:filecount=0`. Default gc_config: `gc*=info,safepoint=info,age*=trace`; overridable via `gc-log-config` param. Provisioner-only.
  - **JitCompiler** (user-selectable, `jit`): Edit the existing OSB `JitCompiler` device so its `instrument_java_opts()` flags are injected into `SOLR_OPTS`; output written to `{log_dir}/jit.log`. Provisioner-only.
  - **Heapdump** (user-selectable, `heapdump`): Edit the existing OSB `Heapdump` device to use the Solr process PID (from `{solr_root}/bin/solr-{port}.pid`) instead of the OpenSearch node PID. At benchmark stop, runs `jmap -dump:format=b,file={log_dir}/heap.hprof {pid}`. Docker variant: `docker exec {container} jmap ...`. Provisioner-only.
  - **DiskIo** (internal, provisioner-only): Edit the existing OSB `DiskIo` device to use the Solr process PID instead of the OpenSearch node PID. Samples `psutil.Process(pid).io_counters()` at benchmark start and stop, records deltas as `disk_io_read_bytes` and `disk_io_write_bytes`.
...
- **FR-019b**: Devices that only apply to SolrCloud mode (`ShardStats`) MUST gracefully detect standalone mode by checking the CLUSTERSTATUS response and skip collection without error.
...
- **FR-019d**: The `SegmentStats` device (existing OSB class, `--telemetry segment-stats`) MUST be edited to replace its OpenSearch `/_cat/segments` call with `GET /solr/{collection}/admin/luke?numTerms=0` (Solr Luke API). The old OpenSearch behavior MUST be removed. It MUST call the Luke API once per collection at benchmark end and store the following metrics per collection: `segment_numdocs` (integer), `segment_maxdoc` (integer), `segment_deleteddocs` (integer), `segment_segmentcount` (integer), `segment_indexheapusagebytes` (bytes). If the Luke API is unavailable for a collection, the device MUST log a WARNING and skip that collection.
- **FR-019e**: A new `ShardStats` device (`--telemetry shard-stats`) MUST be created following OSB naming conventions. It MUST poll on a configurable interval (default 60 seconds). For each active shard leader it MUST call `GET /solr/admin/cores?action=STATUS&core={core}` to retrieve `index.numDocs` and `index.sizeInBytes`. Metrics per shard: `shard_num_docs`, `shard_index_size_bytes`, `shard_state`. On standalone Solr (non-SolrCloud), the device MUST detect the absence of CLUSTERSTATUS collection data and skip without error.
- **FR-019f**: The `FlightRecorder` (`jfr`), `Gc` (`gc`), and `JitCompiler` (`jit`) devices (existing OSB classes) MUST be edited so their `instrument_java_opts() → list[str]` output is injected into `SOLR_OPTS` instead of OpenSearch JVM options files. The JVM flags themselves are JVM-agnostic and remain unchanged. The provisioner MUST append these flags to `SOLR_OPTS` in `_build_env()` / `_cluster_config_env_flags()` before starting Solr. On `benchmark-only` pipeline (no provisioner context), each device MUST log a WARNING and skip flag injection.
- **FR-019g**: The `Heapdump` device (existing OSB class, `--telemetry heapdump`) MUST be edited to read its PID from the Solr PID file (`{solr_root}/bin/solr-{port}.pid`) instead of the OpenSearch node context. At benchmark stop it MUST invoke `jmap -dump:format=b,file={results_dir}/heap.hprof {pid}`. Docker variant: `docker exec {container_name} jmap ...`. On `benchmark-only` (no PID available), it MUST log a WARNING and skip without error.
- **FR-019h**: The `ClusterEnvironmentInfo` device (existing OSB internal class, always-on) MUST be edited to replace its OpenSearch API calls with Solr equivalents. It MUST call `GET /api/node/system` to capture: Solr version string, JVM version string, JVM vendor, OS name, OS version, available processor count. It MUST also query CLUSTERSTATUS to capture: total number of collections, total shard count, total replica count. All values MUST be stored as run-level metadata (not time-series) using the same `add_meta_info()` mechanism. The old OpenSearch endpoint calls MUST be removed. This device works with all pipelines including `benchmark-only`.
- **FR-019i**: The `DiskIo` device (existing OSB internal class, always-on for provisioned pipelines) MUST be edited to read the Solr process PID from `{solr_root}/bin/solr-{port}.pid` instead of the OpenSearch node PID. It MUST use `psutil.Process(pid).io_counters()` to record `read_bytes` and `write_bytes` at `on_benchmark_start()` and again at `on_benchmark_stop()`, storing deltas as `disk_io_read_bytes` and `disk_io_write_bytes`. On `benchmark-only` (no PID), it MUST silently skip without logging a warning.
...
- **FR-026**: Generic framework configuration files (benchmark.ini structure, tox.ini, test runner config) MUST be preserved unless there is a concrete functional reason to change them. The `default.url` in `osbenchmark/resources/benchmark.ini` MUST be updated to `https://github.com/janhoy/solr-benchmark-workloads` so the default workload repository points to the Solr-native workload collection.
...
**5% of code = OpenSearch compatibility (workload detection + standalone conversion tool)**:
- `osbenchmark/solr/conversion/` — workload detection, query DSL → Solr JSON DSL translation, aggregations → Solr facets, schema mapping. This module is ONLY invoked by the `convert-workload` CLI subcommand — never by the benchmark run path.
...
- `osbenchmark/test_run_orchestrator.py` — detects OSB workload format at run start and ABORTS with a clear error if one is detected; does NOT invoke conversion
