...
- [x] T092 Create `docs/` Jekyll scaffold — create `docs/Gemfile` with `gem "jekyll", "~> 4.4.1"` and `gem "just-the-docs", "0.12.0"`; create `docs/_config.yml` per plan.md Phase 1 design (title, theme, url, aux_links, search_enabled, callouts); create `docs/.gitignore` containing `_site/` and `.jekyll-cache/`; verify `cd docs && bundle install && bundle exec jekyll build` succeeds

- [x] T093 [P] Create `.github/workflows/docs.yml` — implement the full GitHub Actions workflow per plan.md Phase 1 design: trigger on push to `main` and `workflow_dispatch`; two jobs (`build` using `ruby/setup-ruby@v1` + `actions/configure-pages@v5` + `bundle exec jekyll build`, and `deploy` using `actions/deploy-pages@v4`); set `working-directory: docs` for all `run` steps; upload artifact from `docs/_site`

- [x] T094 [P] Create `docs/_includes/footer_custom.html` — implement the ASF copyright + attribution footer per plan.md Phase 1 design; include ASF copyright line, Apache 2.0 license link, link to About page, and one-line credit to OpenSearch Benchmark with trademark notice; use `{{ '/about' | relative_url }}` for the About link
...
- [x] T096 [P] [US5] Create `docs/about.md` — license, attribution, and trademark page; front matter: `title: About / Credits`, `nav_order: 102`; sections: (1) License — Apache 2.0 with link to full text; (2) Attribution — "Apache Solr Benchmark is derived from OpenSearch Benchmark (Copyright 2022 OpenSearch Contributors, licensed under Apache 2.0), which in turn derives from Elasticsearch Rally (Copyright Elasticsearch bv)"; (3) Trademarks — "Apache Solr is a trademark of The Apache Software Foundation. OpenSearch® is a registered trademark of the OpenSearch Software Foundation."; (4) Links — apache.org, solr.apache.org, opensearch.org; this is the ONLY page where OpenSearch trademark appears in body text
...
- [x] T110 [US5] Run `cd docs && bundle exec jekyll build --strict` and resolve all warnings and errors — fix any broken internal links, missing `parent` references, duplicate `nav_order` values, or malformed front matter; the build MUST complete with zero warnings

- [x] T111 [P] Terminology audit — scan all docs pages for OSB-specific terms using `grep -r "OpenSearch Benchmark\|create-index\|delete-index\| index \| indices \|aggregation" docs/ --include="*.md" | grep -v about.md`; fix every instance found by applying the adaptation rules from plan.md; verify canonical Solr Benchmark terms used throughout

- [x] T112 [P] Trademark audit — verify OpenSearch trademark appears ONLY in `docs/about.md`; run `grep -r "OpenSearch" docs/ --include="*.md" | grep -v about.md`; any remaining hits (outside license header comments) MUST be removed or replaced with "Apache Solr Benchmark"; confirm `about.md` contains the full trademark notice for both Apache Solr and OpenSearch

- [x] T113 [P] Verify GitHub Actions workflow syntax — run `cat .github/workflows/docs.yml` and confirm YAML is valid; verify `working-directory: docs` is set for all run steps; verify `path: docs/_site` is set on the upload artifact step; optionally validate with `yamllint .github/workflows/docs.yml`
...
| **Phase 16: US3 Remaining Opt-in Devices** | **T130–T144** | **T131–T137, T139–T144** | **US3** |
| **Total** | **144 tasks** | **79 parallelizable** | |
...
- Phase 16 (T130–T144): T130 (list_telemetry) and T138 (provisioner) are sequential prerequisites; T131–T137 (device edits) can all run in parallel with each other; T139–T142 (tests) can run in parallel after T131–T137; T143–T144 (docs) can run in parallel at any time; T132 supersedes T128 (SolrShardStats with wrong naming)
...
### Parallel opportunities (Phase 16)

```
Sequential:   T130  (list_telemetry — no dependencies but logically first)
Parallel:     T131, T132, T133, T134, T135, T136, T137  (device edits — each in different class)
Sequential:   T138  (provisioner — must complete for JVM/PID features to work)
Parallel:     T139, T140, T141, T142  (unit tests — after device edits complete)
Parallel:     T143, T144  (docs — can run alongside any other task)
```

---

## Phase 16: US3 — Remaining Opt-in Telemetry Devices

**Goal**: Port 8 remaining OSB telemetry devices to Solr APIs so users can enable them with `--telemetry`. Three REST-based devices work on all pipelines; five JVM/process devices work only on provisioned pipelines. Implements US3 acceptance scenarios 8, 9, and 10.

**Note**: T132 supersedes T128 (`SolrShardStats` with wrong naming convention). After T132 is complete, T128 should be considered replaced and left unchecked.

**Independent Test**: `solr-benchmark run --pipeline docker --distribution-version 9.10.1 --workload nyc_taxis --telemetry segment-stats,shard-stats,cluster-environment-info --test-mode` — verify `segment_segmentcount` appears in results, SolrCloud check skips gracefully on single-node, `distribution_version` contains "9.".

- [X] T130 [US3] Update `list_telemetry()` in `osbenchmark/telemetry.py` — rewrite the function body: (1) keep the "Solr Benchmark devices (always enabled)" section listing the 6 Solr-native devices from `osbenchmark/solr/telemetry.py`; (2) add a new "Optional REST devices (all pipelines — enable with --telemetry)" section listing `SegmentStats` (segment-stats), `ShardStats` (shard-stats), `ClusterEnvironmentInfo` (cluster-environment-info) with their `.command`, `.human_name`, `.help` attributes; (3) add a "Optional JVM/process devices (provisioned pipelines only — docker or from-distribution)" section listing `FlightRecorder` (jfr), `Gc` (gc), `JitCompiler` (jit), `Heapdump` (heapdump), `DiskIo` (disk-io); (4) remove the "OpenSearch-only devices" section entirely (these devices are now ported to Solr); (5) add a footer note: "JVM/process devices inject flags into SOLR_OPTS before Solr starts and are silently skipped on the benchmark-only pipeline."
- [X] T131 [P] [US3] Edit `SegmentStats` in `osbenchmark/telemetry.py` — update the constructor to accept `host: str, port: int` instead of `client` (the OpenSearch client); update `on_benchmark_stop()`: replace `self.client.cat.segments()` with a `requests.get(f"http://{host}:{port}/solr/admin/collections?action=LIST")` call to get collection names, then for each collection call `requests.get(f"http://{host}:{port}/solr/{coll}/admin/luke?numTerms=0&wt=json")` and extract `index.numDocs`, `index.maxDoc`, `index.deletedDocs`, `index.segmentCount`, `index.sizeInBytes`; write one JSON line per collection to `{log_root}/segment_stats.log`; log path to console with `console.info()`; wrap entire body in `try/except BaseException` to prevent telemetry failure from aborting the benchmark.
- [X] T132 [P] [US3] Edit `ShardStats` in `osbenchmark/telemetry.py` — replace the class body (keep `command = "shard-stats"`, `human_name`, `help`; remove `warning` and `opensearch_distribution_name` class attributes); new constructor `__init__(self, telemetry_params, admin_client, metrics_store)` where `admin_client` is a `SolrAdminClient`; remove references to `self.clients` dict; new `on_benchmark_start()`: call `GET /solr/admin/collections?action=CLUSTERSTATUS` via `admin_client._get_session().get(...)`; if response JSON has no `"cluster"` key or no `"collections"` under `cluster` → `console.info("ShardStats: not a SolrCloud cluster — skipping shard stats")` and return without starting samplers; if SolrCloud: start `SamplerThread` with a `ShardStatsRecorder(admin_client, metrics_store, sample_interval)` where `ShardStatsRecorder.__call__()` iterates CLUSTERSTATUS collections/shards, for each shard leader calls `GET /solr/admin/cores?action=STATUS&core={core_name}`, emits `metrics_store.put_value_cluster_level(f"shard_{shard_name}_num_docs", status[core]["index"]["numDocs"], "docs")` and `metrics_store.put_value_cluster_level(f"shard_{shard_name}_size_bytes", status[core]["index"]["sizeInBytes"], "byte")`; new `on_benchmark_stop()` stops all samplers.
- [X] T133 [P] [US3] Edit `ClusterEnvironmentInfo` in `osbenchmark/telemetry.py` — update the constructor to accept `admin_client` (SolrAdminClient) instead of `client` (OpenSearch client); replace `on_benchmark_start()` body: call `requests.get(f"{admin_client.base_url}/api/node/system")` (use `admin_client._get_session()`) and extract `lucene.solr-spec-version` → `metrics_store.add_meta_info(MetaInfoScope.cluster, None, "distribution_version", ...)`, `jvm.version` → `"jvm_version"`, `jvm.name` → `"jvm_vendor"`, `system.availableProcessors` → `"cpu_logical_cores"`; call CLUSTERSTATUS and count `len(cluster.liveNodes)` → `"node_count"`; remove calls to `store_plugin_metadata`, `store_node_attribute_metadata`, `self.client.nodes.info()` (all OpenSearch-specific); wrap in `try/except BaseException` with a warning log on failure.
- [X] T134 [P] [US3] Edit `FlightRecorder` in `osbenchmark/telemetry.py` — in `instrument_java_opts()`: (1) add pipeline-skip guard at top: `if self.telemetry_params.get("pipeline", "benchmark-only") == "benchmark-only": self.logger.warning("FlightRecorder: skipping — not supported on benchmark-only pipeline"); return []`; (2) remove the Java < 11 commercial license warning block (the multiline `console.println` block with the Oracle URL and `time.sleep(3)`); (3) remove the `java_major_version < 11` commercial-feature flag `"-XX:+UnlockCommercialFeatures"` from `java_opts()`; (4) remove the `java_major_version < 9` legacy code path (keep only the modern `"-XX:StartFlightRecording=maxsize=0,maxage=0s,disk=true,dumponexit=true,filename={log_file}"` path); update console message from "OSB" to "Solr Benchmark" if referenced.
- [X] T135 [P] [US3] Edit `Gc` in `osbenchmark/telemetry.py` — in `instrument_java_opts()`: (1) add pipeline-skip guard at top: `if self.telemetry_params.get("pipeline", "benchmark-only") == "benchmark-only": self.logger.warning("Gc: skipping — not supported on benchmark-only pipeline"); return []`; (2) in `java_opts()`, remove the `java_major_version < 9` branch that returns the legacy `-Xloggc:` flag list; keep only the `else` branch (`-Xlog:` JDK 9+ format) and remove the `if/else` entirely (always use modern format); keep `gc-log-config` telemetry param support.
- [X] T136 [P] [US3] Edit `JitCompiler` in `osbenchmark/telemetry.py` — in `instrument_java_opts()`: add pipeline-skip guard at top: `if not hasattr(self, "telemetry_params") or self.telemetry_params.get("pipeline", "benchmark-only") == "benchmark-only": self.logger.warning("JitCompiler: skipping — not supported on benchmark-only pipeline"); return []`; update constructor to accept an optional `telemetry_params` dict (default `{}`); otherwise return the existing flag list unchanged.
- [X] T137 [P] [US3] Edit `Heapdump` in `osbenchmark/telemetry.py` — update constructor to accept optional `docker_container: str | None = None` parameter (for Docker pipeline); in `detach_from_node(node, running)` if `running`: build `heap_dump_file` path as before; if `self.docker_container` is set → build cmd as `docker exec {self.docker_container} jmap -dump:format=b,file={heap_dump_file} {node.pid}`; otherwise use the existing `jmap ...` command; wrap subprocess call in `try/except BaseException` (log warning on failure but do not raise); replace remaining "OSB" references in log/console messages with "Solr Benchmark".
- [X] T138 [US3] Update `SolrProvisioner` in `osbenchmark/solr/provisioner.py` — (1) in `start()`, after `_wait_for_ready()` completes, read the PID file at `os.path.join(self.install_root, "bin", f"solr-{port}.pid")`: if file exists → `self.node.pid = int(open(pid_path).read().strip())`; if absent → `self.node.pid = None` with `self.logger.warning("Could not read Solr PID file at %s", pid_path)`; (2) in `_build_env()`, accept an optional `telemetry_devices: list = None` parameter; iterate devices and call `device.instrument_java_opts()` if the method exists; collect returned string lists and append space-separated to `env["SOLR_OPTS"]`; (3) apply same JVM opts injection in `SolrDockerLauncher._cluster_config_env_flags()`: accept `telemetry_devices` parameter and append to `SOLR_OPTS` env flag; (4) for Docker PID after `docker run`: after container starts call `docker inspect {container_name} --format='{{{{.State.Pid}}}}'` via `subprocess.check_output` and store in `self.node.pid`; wrap in try/except.
- [X] T139 [P] [US3] Write unit tests for `SegmentStats` Solr Luke API in `tests/unit/test_telemetry.py` — `test_segment_stats_luke_success`: mock `requests.get` for collections LIST returning `{"collections": ["my_coll"]}` and luke response returning `{"index": {"numDocs": 1000, "maxDoc": 1001, "deletedDocs": 1, "segmentCount": 5, "sizeInBytes": 204800}}`; call `device.on_benchmark_stop()`; assert the log file at `log_root/segment_stats.log` is created and contains `"numDocs"` and `"my_coll"`; `test_segment_stats_luke_failure_graceful`: mock luke returning HTTP 500; assert `on_benchmark_stop()` does not raise any exception.
- [X] T140 [P] [US3] Write unit tests for `ShardStats` Solr CLUSTERSTATUS in `tests/unit/test_telemetry.py` — `test_shard_stats_solrcloud`: construct device with mock `SolrAdminClient`; mock CLUSTERSTATUS returning `{"cluster": {"collections": {"my_coll": {"shards": {"shard1": {"replicas": {"core_node1": {"state": "active", "leader": "true", "core": "my_coll_shard1_replica_n1"}}}}}}}}`; mock core STATUS returning `{"status": {"my_coll_shard1_replica_n1": {"index": {"numDocs": 500, "sizeInBytes": 10240}}}}`; call sampler's `__call__()`; assert `put_value_cluster_level` called with metric name `"shard_shard1_num_docs"` and value 500; `test_shard_stats_standalone`: mock CLUSTERSTATUS returning `{}` (no `cluster` key); assert `samplers` list remains empty after `on_benchmark_start()`.
- [X] T141 [P] [US3] Write unit tests for `ClusterEnvironmentInfo` Solr system API in `tests/unit/test_telemetry.py` — `test_cluster_env_info_solr_version`: mock `admin_client` GET to `/api/node/system` returning `{"lucene": {"solr-spec-version": "9.7.0"}, "jvm": {"version": "21.0.1", "name": "OpenJDK 21"}, "system": {"availableProcessors": 8}}`; mock CLUSTERSTATUS with `{"cluster": {"liveNodes": ["127.0.0.1:8983_solr"]}}`; call `device.on_benchmark_start()`; assert `metrics_store.add_meta_info` called with `"distribution_version"` = `"9.7.0"` and `"jvm_version"` = `"21.0.1"`; `test_cluster_env_info_failure_graceful`: mock GET raising `requests.ConnectionError`; assert `on_benchmark_start()` does not raise.
- [X] T142 [P] [US3] Write unit tests for JVM device pipeline-skip behavior in `tests/unit/test_telemetry.py` — `test_jfr_benchmark_only_skip`: construct `FlightRecorder({"pipeline": "benchmark-only"}, log_root="/tmp", java_major_version=21)`; assert `instrument_java_opts()` returns `[]`; `test_jfr_docker_returns_flags`: construct with `{"pipeline": "docker"}`; assert return value is a non-empty list containing a string with `"-XX:StartFlightRecording="`; `test_gc_benchmark_only_skip`: construct `Gc({"pipeline": "benchmark-only"}, log_root="/tmp", java_major_version=21)`; assert `instrument_java_opts()` returns `[]`; `test_gc_docker_returns_flags`: construct with `{"pipeline": "docker"}`; assert return contains `-Xlog:`; `test_jit_benchmark_only_skip`: construct `JitCompiler("/tmp", telemetry_params={"pipeline": "benchmark-only"})`; assert `instrument_java_opts()` returns `[]`.
- [X] T143 [P] [US3] Update `docs/reference/telemetry.md` — after the existing 6 always-on device sections add a horizontal rule and a new section `## Optional devices`; add intro paragraph: "Optional devices must be explicitly requested with `--telemetry <name>`. REST-based optional devices work with all pipelines. JVM/process devices inject flags into `SOLR_OPTS` before Solr starts and require the `docker` or `from-distribution` pipeline — they are silently skipped on `benchmark-only`."; add one `###` subsection per device: `### segment-stats` (REST, all pipelines, Luke API, metrics: segment_numdocs, segment_segmentcount, segment_deleteddocs, segment_sizeInBytes), `### shard-stats` (REST, all pipelines, SolrCloud only — skipped silently on standalone, metrics: shard_{name}_num_docs, shard_{name}_size_bytes), `### cluster-environment-info` (REST, all pipelines, stores distribution_version, jvm_version, jvm_vendor, cpu_logical_cores as run metadata), `### jfr` (JVM, provisioned only, writes profile.jfr to benchmark log dir), `### gc` (JVM, provisioned only, writes gc.log with -Xlog: format), `### jit` (JVM, provisioned only, writes jit.log), `### heapdump` (process, provisioned only, writes heap_at_exit_{pid}.hprof on benchmark stop), `### disk-io` (process, always active on provisioned runs, records disk_io_read_bytes and disk_io_write_bytes); also update the "Using multiple devices" example to show both always-on and optional device names.
- [X] T144 [P] [US3] Update `docs/user-guide/understanding-results/telemetry.md` — expand "Available devices" table into two separate tables: (1) "Always-enabled devices" with the existing 6 rows; (2) "Optional devices (enable with --telemetry)" with 8 rows for segment-stats, shard-stats, cluster-environment-info, jfr, gc, jit, heapdump, disk-io — each with pipeline requirement column (All / Provisioned only); add a new "## Optional telemetry devices" section with prose explaining: when to use segment-stats (debugging segment fragmentation), shard-stats (SolrCloud distribution), cluster-environment-info (audit metadata), JVM devices (profiling during provisioned runs); add example command: `solr-benchmark run --pipeline benchmark-only --target-hosts localhost:8983 --workload nyc_taxis --telemetry segment-stats,shard-stats`; add note: "JVM devices (jfr, gc, jit, heapdump, disk-io) silently skip when pipeline is benchmark-only — no warning is shown in the results."

**Checkpoint Phase 16**: All 8 opt-in devices are functional; SegmentStats writes segment_stats.log; ShardStats polls CLUSTERSTATUS on SolrCloud and skips gracefully on standalone; ClusterEnvironmentInfo records distribution_version from /api/node/system; FlightRecorder/Gc/JitCompiler return [] on benchmark-only pipeline; provisioner sets node.pid from Solr PID file; docs cover all 14 devices.
